...
import { getNewQuestion, getNextLongDegree } from 'src/utils'
...
export default function QuestionsList() {
  ...
  const { questions, addQuestion, editExam } = useDB((state) => ({
    ...
    addQuestion: () =>
      state.addQuestion(openedExamId, getNewQuestion()),
    editExam: (data: Partial<ExamType>) =>
      state.editExam(openedExamId, data),
  }))
...
const QuestionEditor = ({
  ...
  const { editQuestion, deleteQuestion } = useDB((state) => ({
    ...
    deleteQuestion: () =>
      state.deleteQuestion(openedExamId, question.id),
  }))
  const nextLongDegree = getNextLongDegree(
    Object.keys(question.longBareme)
  )
        ...
        <div className='grid grid-cols-2 gap-6 w-full bg-indigo-50 py-4 pl-4 pr-4 sm:pr-6'>
          ...
          {question.type === 'long' && (
            <div className='col-span-2 flex gap-6 items-center justify-end overflow-auto px-[2px]'>
              {Object.entries(question.longBareme).map(
                ([degree, value]) => (
                  <div className=''>
                    <label
